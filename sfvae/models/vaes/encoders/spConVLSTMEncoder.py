...
@gin.configurable('SpConvLSTMEncoder')
class SpConvLSTMEncoder(nn.Module):
    def __init__(self, layer_dim_list=gin.REQUIRED, activation='ReLU', act_param=[]):
        super(SpConvLSTMEncoder, self).__init__()
        ...
        '''
        self.input_dim, self.hidden_dim, self.kernel_size, self.num_layers, self.latent_size, self.convlstm_input_size = layer_dim_list[-1]
        ...
        self.after_conv_size = self.hidden_dim[-1] * self.convlstm_input_size[0] * self.convlstm_input_size[1]
        self.linear_means = nn.Linear(self.after_conv_size, self.latent_size)
        self.linear_log_var = nn.Linear(self.after_conv_size, self.latent_size)
        # self.weight_init()
    ...
    def forward(self, x):
        ...
        x = self.layers(x)
        x = x.view(batch_size, seq_len, self.hidden_dim[0], self.convlstm_input_size[0], self.convlstm_input_size[1])
        _, pred = self.convlstm(x)
        final_state = pred[0][0].view(batch_size, self.after_conv_size)
